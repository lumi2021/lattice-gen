...
func to_svg() -> SvgElement:
	...
	var draw_count = _get_draw_count(length)
	var obstacle_boxes = _get_obstacle_bounding_boxes()
	var global_xform = get_global_transform()
	...
	for i in range(draw_count):
		...
		# Adiciona o parafuso principal se não colidir
		if not _is_point_inside_obstacles(self.position + screw_pos, screw_radius, obstacle_boxes):
			var hole = SvgCircle.new()
			hole.transform = global_xform
			hole.center = screw_pos
			hole.radius = screw_radius
			hole.operation = "cut"
			hole.fill = "none"
			hole.stroke = "#00FF00"
		...
		# Adiciona os furos guia se ativado
		if guides:
			var is_outer_column = (i == 0 or i == draw_count - 1)
			if hide_outer_guides and is_outer_column: continue
			...
			for guide_pos in guide_positions:
				if not _is_point_inside_obstacles(self.position + guide_pos, guide_radius, obstacle_boxes):
					var guide_hole = SvgCircle.new()
					guide_hole.transform = global_xform
					guide_hole.center = guide_pos
					guide_hole.radius = guide_radius
					guide_hole.operation = "cut"
					guide_hole.fill = "none"
					guide_hole.stroke = "#00FF00"
					guide_hole.stroke_width = 1.0
					g.elements.append(guide_hole)
